...
% Variations discussed include the imposition of conditions on the factors; for instance, the problem of computing an approximate Singular Value Decomposition, or an approximate (rank-revealing) QR factorization. Randomized techniques have proven highly effective at solving such approximation problems in a range of different environments, for instance: (1) For a general dense matrix $\mtx{A}$, accurate and stable randomized techniques require $\O(mn\log(k))$ operations, as opposed to the $O(mnk)$ operations required by classical techniques. (2) For sparse matrices, and other matrices that can rapidly be applied to vectors, randomized techniques match the number of floating point operations required by state-of-the-art classical techniques such as Arnoldi or Lanczos, but are more robust, and allow much more flexibility in organizing the computation, allowing multi-core architectures to be exploited fully. (3) For very large matrices that do not fit in fast memory, randomized methods allow approximate factorizations to be computed out-of-core with as few as a single pass over the data.
...
% Low-rank matrix approximations, such as the truncated singular value decomposition and the rank-revealing QR decomposition, play a central role in data analysis and scientific computing. Randomization offers a powerful tool for performing low-rank matrix approximation.
...
% This formulation of low-rank approximation is a template for important 
...
% The first is to construct a low-dimensional subspace that captures the action of the matrix. The second is to restrict the matrix to the subspace and then compute a standard factorization (QR, SVD, etc.) of the reduced matrix.
...
The basic challenge in producing low-rank matrix approximations is a primitive question that is called the \term{fixed-precision approximation problem} \cite{RM}:
...
% But, the smallest possible $k$ might not be computationally interesting. So, we can restate the problem in two forms.

% \begin{subdefn}
% \label{problem:fixed_precision_tight}
% Given a matrix $\mtx{A}$ and a tolerance $\epsilon$, we seek a matrix $\mtx{Q}_{(k)}$ with $k$ orthonormal columns which is a solution to the optimization problem below:
% \begin{equation*}
% \begin{aligned}
% & \underset{\mtx{Q}_{(k)}}{\text{minimize}}
% & & k \\
% & \text{subject to}
% & & \norm{ \mtx{A} - \mtx{Q}_{(k)}\mtx{Q}_{(k)}^\adj \mtx{A} } \leq \epsilon.
% \end{aligned}
% \end{equation*}
...
\begin{problem}[Fixed-Rank Approximation]
Given a matrix $\mtx{A}$ and a target rank $k$,
% and an oversampling parameter $p$, 
...
% Fortunately, the Householder QR factorization procedure \cite[Algorithm 5.2.1]{golubvan} can be modified in a simple way to produce an orthonormal basis for range of $\mtx{A}$. The modified algorithms computes the factorization 
...
% \paragraph{Truncated Pivoted QR Factorization}
...
% \paragraph{Computational Complexity}
% The permutation matrix $\mtx{P}$ is determined by column pivoting such that $\mtx{R}_{11}^{(k)}$ is well conditioned and $\mtx{R}_{22}^{(k)}$ is negligible (the larger entries of $\mtx{R}$ are moved to the upper left corner and the smallest entries are isolated in the bottom submatrix). 
% This decomposition is computed by the algorithm described in \cite{golubvan,Stewartbook} requiring $\O(kmn)$ flops.
...
% To understand how randomness helps solving the fixed-rank problem, it is helpful to consider some motivating examples \cite{RM}.
...
% to general matrices. The procedure can formalize in the Algorithm \ref{alg:proto}.
...
\begin{center}
\textsc{Algorithm \ref{alg:proto}: Randomized Projective Algorithm for Solving the Fixed-Rank Problem}
\end{center}
...
\anum{3} \>Construct a matrix $\mtx{Q}$ whose columns form an orthonormal basis for \\
         \>the range of $\mtx{Y}$ (\textit{e.g.} by SVD on $\mtx{Y}$).
\end{tabbing}
\end{minipage}}
\end{center}
\end{figure}
...
Stage A using Algorithm~\cite[Algorithm~4.1]{RM} %\ref{alg:basic} 
with $p$ constant
(for the fixed-rank problem)
or Algorithm~\cite[Algorithm 4.2]{RM} %\ref{alg:adaptive2} 
...
% as discussed in \cite{RM}%\S\S\ref{sec:algorithm}--\ref{sec:otherfactorizations}.

\begin{figure}
\begin{center}
\framebox{\begin{minipage}{.9\textwidth}
\begin{center}
\textsc{Algorithm 3: Prototype for Randomized SVD}
\end{center}
...
%Our numerical experience suggests that
...
% or when the input matrix is very large.
...
% interfere with the calculation, so by reducing their weight relative to
...
{\bf Note:} The recommended implementation appears as Algorithm~\ref{alg:subspaceiteration}.
\end{minipage}}
\end{center}
%\caption{trash}
...
matrix--vector multiplies as Algorithm 4.1 in \cite{RM}, but is far
more accurate in situations where the singular values of $\mtx{A}$
decay slowly. See  \cite[Thm 9.2]{RM} for the details.

% Algorithm~\ref{alg:poweriteration} targets the fixed-rank problem.  To address the fixed-precision
% problem, the author incorporated the error estimators described in~\cite[]{}%\S\ref{sec:aposteriori}
...
% Algorithm~\ref{alg:subspaceiteration} is more useful because its computational costs are similar to
...
% stored.  
This section describes a method in \cite{RM} which requires just
...
$\norm{\mtx{A} - \mtx{Q}\mtx{Q}^{\adj}\mtx{A}} \leq \eps$ and $\mtx{Y} = \mtx{Q}\mtx{Q}^{\adj}\mtx{Y}$,
...
concentration effects.  % in~\S\ref{sec:prob-failure},
...
% In this section, we present bounds on the performance of Algorithm \ref{alg:proto} when it is implemented with an SRFT test matrix.
...
% random dimen\-sion-reduction map.  In other words, it transports a
...
% $$
% with failure probability at most $\bigO(k^{-1})$. ~\cite{Tro10:Improved-Analysis}
% \end{theorem}
...
% bound on $\ell$ cannot generally be removed.
...
% ~\S\ref{sec:gauss-avg-case},
% The quantity $\sigma_{k+1}$ is the minimal spectral-norm error possible
% when approximating $\mtx{A}$ with a rank-$k$ matrix \cite[\S 10.2]{RM}.
...
%Indeed, SRFTs sometimes perform even \textit{better}
